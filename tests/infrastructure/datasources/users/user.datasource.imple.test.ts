...
vi.mock('../../../../prisma/prisma.connection', () => ({
  default: {
    users: {
      create: vi.fn(),
      update: vi.fn(),
...
describe('UserDatasourceImpl', () => {
  ...
  it('should retrieve all users', async () => {
    ...
    const mockUserEntities = [new UserEntity(1, 'User1'), new UserEntity(2, 'User2')];

    prisma.users.findMany.mockResolvedValue(mockPrismaUsers);
    UserMapper.toDomain.mockImplementation(user => mockUserEntities.find(e => e.id === user.id));
  ...
  it('should throw error if user not found by id', async () => {
    ...
    await expect(datasource.getById(1)).rejects.toThrow(new CustomGeneralError('User with id: 1 not found', 404));
  });
